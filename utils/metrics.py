...
def init_statsd():
    ...
    logger.info("statsd client initialized successfully for host: %s:%s", host, port)
...
@contextmanager
def timing_context(stat_name: str):
    """Context manager to measure execution time (in ms) and send to statsd under `stat_name`"""
    start_time = time.perf_counter()
    try:
        yield
    finally:
